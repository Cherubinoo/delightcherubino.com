      ...
      <div style={{ padding: '1.5rem', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div style={{ 
          ...
          {project.image ? (
            ...
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(45deg, rgba(255,255,255,0.02), transparent)' }}>
               <Briefcase size={48} style={{ opacity: 0.1 }} />
            </div>
          )}
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', color: 'var(--accent)' }}>
              <Briefcase size={14} />
              <span className="mono" style={{ fontSize: '0.7rem', letterSpacing: '0.1em' }}>{project.company || "COMMERCIAL PROJECT"}</span>
            </div>
            {project.award_name && <span title={project.award_name} style={{ fontSize: '1.2rem' }}>🏆</span>}
          </div>
  ...
  return (
    <section id="work" className="section">
      <div className="container">
        ...
        >
          <div className="mono" style={{ color: 'var(--accent)', marginBottom: '1rem' }}>Selected Case Studies</div>
          <h2 className="oversized-text">Commercial <span style={{ color: 'var(--muted)' }}>Impact.</span></h2>
        </motion.div>
              ...
              <div className="modal-grid">
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', color: 'var(--accent)', marginBottom: '2.5rem' }}>
                    <Briefcase size={20} />
                    <span className="mono" style={{ fontSize: '1rem', letterSpacing: '0.1em' }}>{selectedProject.company}</span>
                  </div>
                  {selectedProject.award_name && (
                    <div className="pill" style={{ background: '#FFD700', color: '#000', marginBottom: '2rem' }}>
                      🏆 {selectedProject.award_name}
                    </div>
                  )}
                ...
                <div className="modal-sidebar">
                  ...
                  <div>
                    <h4 className="mono" style={{ marginBottom: '2rem', opacity: 0.4, fontSize: '0.8rem', letterSpacing: '0.2em' }}>STATUS</h4>
                    <p style={{ fontSize: '1.5rem', fontWeight: 800 }}>{selectedProject.is_ongoing ? "Active" : "Completed"}</p>
                  </div>
                </div>
              </div>
            </motion.div>
